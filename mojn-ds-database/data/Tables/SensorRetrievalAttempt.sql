CREATE TABLE [data].[SensorRetrievalAttempt] (
    ...
    [IsSensorRetrieved]      TINYINT       NOT NULL,
    ...
    CONSTRAINT [CK_SensorRetrievalAttempt_OneSuccessfulRetrievalPerDeployment] CHECK ([qa].[Sensor_Validate_OneSuccessfulRetrievalPerDeployment]([VisitID],[SensorDeploymentID],[RetrievalTimeOfDay],[IsSensorRetrieved])=(1)),
    ...
    CONSTRAINT [FK_SensorRetrievalAttempt_IsSensorRetrieved] FOREIGN KEY ([IsSensorRetrieved]) REFERENCES [lookup].[IsSensorRetrieved] ([ID]),
...
GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Was sensor retreived (Y/N)', @level0type = N'SCHEMA', @level0name = N'data', @level1type = N'TABLE', @level1name = N'SensorRetrievalAttempt', @level2type = N'COLUMN', @level2name = N'IsSensorRetrieved';
