CREATE VIEW intermediate.VisitpHCalibrationCrosswalk
AS
...
FROM
  (SELECT sv.VisitID,
   ...
   INNER JOIN data.CalibrationpH_Shared AS cal ON wq.pHInstrumentID = cal.pHInstrumentID) AS all_cal
INNER JOIN
  (SELECT sv.VisitID,
   ...
   INNER JOIN data.WaterQualityActivity AS wq ON wq.VisitID = sv.VisitID
   INNER JOIN lookup.WaterQualityDataCollected AS wqc ON wq.WaterQualityDataCollectedID = wqc.ID
   INNER JOIN data.CalibrationpH_Shared AS cal ON wq.pHInstrumentID = cal.pHInstrumentID
...
Begin DesignProperties = 
   ...
   End
End
', @level0type = N'SCHEMA', @level0name = N'intermediate', @level1type = N'VIEW', @level1name = N'VisitpHCalibrationCrosswalk';


GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'intermediate', @level1type = N'VIEW', @level1name = N'VisitpHCalibrationCrosswalk';
